...
import { ArrowLeft, Upload, Eye, Save, X, Check, Image as ImageIcon, MoreHorizontal } from 'lucide-react';
...
const CreateCamperPost = () => {
  ...
  useEffect(() => {
    if (camperData.content.length > 0) {
      ...
      const firstParagraph = camperData.content.find(item => item.type === 'p');
      const firstContent = camperData.content[0];
      
      // Use the paragraph text or the first content text
      const excerptText = firstParagraph ? firstParagraph.text : firstContent.text;
      
      // Truncate to 150 characters
      const truncatedText = excerptText.length > 150 
  ...
  const addContent = () => {
    ...
    switch(contentType) {
      ...
      const validateForm = () => {
        ...
        if (!camperData.featuredImage && !imageFile) return 'Featured image is required';
      ...
      const handleSave = async () => {
        ...
        try {
          ...
          const cloudinaryUrl = imageFile ? await uploadImage() : camperData.featuredImage;
                ...
                <div className="relative w-full h-[60vh]">
                  ...
                  <div className="absolute inset-0 bg-black bg-opacity-40 flex flex-col items-center justify-center text-center">
                    <h1 className="text-4xl md:text-5xl font-bold text-white mb-4">
                      {camperData.imageTitle || camperData.title || 'Untitled Camper'}
                    </h1>
            ...
            <div className="max-w-4xl mx-auto">
                        ...
                        <label className="flex flex-col items-center justify-center cursor-pointer">
                          {uploading ? (
                            <div className="flex flex-col items-center">
                              ...
                              <span className="text-gray-600 font-medium">Uploading to Cloudinary...</span>
                            </div>
                          ) : (
                            <>
              ...
              <section className="mb-8 bg-white p-6 rounded-lg border border-gray-200">
                ...
                <div className="mb-4">
                  ...
                  {contentType && (
                    <div className="p-4 bg-gray-50 rounded-lg mb-4 border border-gray-200">
                      ...
                      <div className="space-y-3">
                        <div>
                          <label className="block text-sm font-medium text-gray-700 mb-1">
                            {contentType === 'link' || contentType === 'cta' ? 'Text to Display' : 'Content Text'}
                          </label>
                          <input
                            type="text"
                            value={contentText}
                  ...
                  {/* Content Preview */}
                  <div>
                    <h3 className="font-medium text-gray-800 mb-3">Content Elements</h3>
                    
                    {camperData.content.length === 0 ? (
                      <div className="text-center py-8 bg-gray-50 rounded-lg border border-gray-200">
                        <p className="text-gray-500">No content elements added yet</p>
                        <p className="text-gray-400 text-sm mt-1">Use the buttons above to add content</p>
                      </div>
                    ) : (
                      <div className="space-y-3 max-h-[400px] overflow-y-auto p-4 bg-gray-50 rounded-lg border border-gray-200">
                        {camperData.content.map((item, index) => (
                          <div key={index} className="flex items-start justify-between p-3 bg-white rounded-lg shadow-sm border border-gray-100">
                            <div className="flex items-center gap-3 flex-1 min-w-0">
                              <span className={`px-2 py-1 rounded text-xs font-medium ${
                                item.type === 'h1' ? 'bg-purple-100 text-purple-800' :
                                item.type === 'h2' ? 'bg-blue-100 text-blue-800' :
                                item.type === 'p' ? 'bg-green-100 text-green-800' :
                                item.type === 'link' ? 'bg-yellow-100 text-yellow-800' :
